...
const PromptUnitTester: React.FC<PromptUnitTesterProps> = ({ prompt }) => {
    ...
    return (
        <div className="bg-card p-4 rounded-lg space-y-4 border border-border-color mt-4">
            <div className="flex justify-between items-center">
                <div>
                    <h3 className="text-lg font-semibold text-text-main">{t('tester_title')}</h3>
                    <p className="text-sm text-text-secondary">{t('tester_desc')}</p>
                </div>
                <button
                    onClick={runTests}
                    disabled={isRunning}
                    ...
                    <div key={tc.id} className="flex items-center gap-3 p-3 bg-background rounded-md">
                        ...
                        <div className="flex-grow">
                            ...
                            <input
                                ...
                                className="w-full mt-1 p-2 bg-card-secondary border border-border-color rounded-md text-sm text-text-main"
                            />
                        </div>
                        <div className="flex-grow">
                             <label className="text-xs text-text-secondary">{t('tester_expected_output_label')}</label>
                            <input
                                type="text"
                        ...
                        <button onClick={() => removeTestCase(tc.id)} className="p-2 text-text-secondary hover:text-red-400">
            ...
            {results.length > 0 && (
                 <div className="pt-4">
                    <h4 className="text-md font-semibold text-text-main mb-2">{t('tester_results_title')}</h4>
                    <ul className="space-y-2">
                        {results.map(result => {
                            const testCase = testCases.find(tc => tc.id === result.testCaseId);
                            return (
                                <li key={result.testCaseId} className="p-3 bg-background rounded-md">
                                    <div className="flex items-center justify-between">
                                        <p className="text-sm font-mono text-text-secondary truncate" title={testCase?.input}>Input: "{testCase?.input}"</p>
                                        {result.pass ? (
                                    ...
                                    <p className="text-xs text-text-secondary mt-2">Expected to contain: "{testCase?.expectedOutput}"</p>
                                    <p className="text-xs text-text-secondary mt-1">Actual: "{result.actualOutput}"</p>
