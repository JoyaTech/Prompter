...
function App() {
  ...
  const handleAcceptChallenge = (challengeText: string) => {
    setChallengeToLoad(challengeText);
    setCurrentPage('editor');
  };

  const onChallengeLoaded = () => {
        ...
        <main className="flex-1 overflow-y-auto p-8">
          {currentPage === 'editor' && (
            <PromptEditor
              ...
              onChallengeLoaded={onChallengeLoaded}
              t={t}
            />
          )}
          {currentPage === 'dashboard' && <Dashboard history={history} t={t} />}
          {currentPage === 'theme' && <ThemeCustomizer />}
          {currentPage === 'challenge' && <ChallengePage onAcceptChallenge={handleAcceptChallenge} t={t} />}
        </main>
      </div>
    </div>
  );
